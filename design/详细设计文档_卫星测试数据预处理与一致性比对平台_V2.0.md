...
`group_id` 指向 4.1.5a 的卫星分组节点；模板对该分组及其所有后代分组下的卫星可用（详见 6.2.3.2）。
...
| 海量接口适配 | `MassDataApiClient` | `GET /api/mass-data/satellites`、`POST /api/mass-data/basic/parameters`、`POST /api/mass-data/satellite/config` | 封装 Web API 调用、超时与重试、错误码映射 |
| 卫星资产适配 | `SatelliteAssetApiClient` | 卫星资产服务「测试阶段」接口 | 获取每颗星的测试阶段清单（编号、阶段名、起止时间） |
| 资产同步编排 | `AssetSyncService` | 上述两个 Client + PostgreSQL | 按「全量卫星 → 每星参数 → 每星测试阶段 → 每星 Mongo 配置」的固定三步骤编排同步、维护增量游标与单星失败隔离 |
...
数据表引用：`data_source_config`、`satellite_cache`、`param_cache`、`test_batch_cache` 见第 4 章。`test_batch_cache` 在本阶段承载「测试阶段」语义，`test_batch_id` 即为阶段编号。

#### 6.1.2 资产同步流程

同步流程严格按以下三步骤执行：先通过海量接口拿全量卫星列表确定 `(taskNo, satNo, dbStage)` 三元组，再以三元组为入参分别拉取每颗星的参数元数据与测试阶段，最后补取 Mongo 连接配置完成 `satellite_cache` 字段闭环。
...
| 模板名称 | 文本框 | 必填，同分组下唯一 |
| 描述 | 多行文本 | 可选 |
| 适用数据范围 | **分组下拉树**（取代原型中的 `taskNo/satNo` 平铺下拉） | 必选，单选；下拉项为分组多级树（可展开/折叠），选中后将本模板归属到该分组；选中节点会显示该分组及其后代的卫星预览（折叠列表） |

> 与原型差异：原型 `<select>` 中是 `TASK-A100 / SAT-001` 一类的（任务号/卫星号）平铺项；新设计改为分组树下拉（推荐组件 antd `<TreeSelect>`），仅可选择分组节点，不再允许直接挂在某颗卫星上。

**有效时间段提取规则块**（保留原型语义）：

- 逻辑组合：参数条件可以多行，行间通过下拉切换 `AND` / `OR`；支持嵌套组形成 `ruleTree`；
- 单条参数条件：`参数（来自 param_cache）`+`比较符（>, >=, <, <=, ==, !=, between）`+`阈值`；
...
| 列 | 说明 |
|---|---|
| 提取目标参数 | 从 `param_cache` 选择，显示 `param_id` + `param_name` + 单位 |
