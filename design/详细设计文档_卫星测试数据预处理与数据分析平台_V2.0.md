...
| `IConditionHistoryProvider` | 运行期从海量接口拉条件历史：参数 `query/parameters`、指令 `query/instructions` |
...
**条件历史数据来源**（运行期）：
- 参数条件：海量 `POST /api/v2/mass-data/query/parameters`；
- 指令条件：海量 `POST /api/v2/mass-data/query/instructions`（对应“查询指令表”运行期历史查询）；
...
> 说明：`POST /api/v2/mass-data/query/parameters` 与 `POST /api/v2/mass-data/query/instructions` 用于运行期历史数据查询（有效时间段提取与指令条件求值），不参与资产同步。
...
| V2.0.10 | 2026-06-16 | **筛选模板规则升级 + 海量接口 v2 统一**：6.2.3 从 `ruleTree` 主模型升级为 `conditionConfig`（指令条件 + 参数条件 + 表达式 `&&/||/()`），参数比较符统一为 `> >= < <= = != between`；配置期候选来源明确为 `command_cache` / `param_cache`，运行期条件历史数据改为海量 `POST /api/v2/mass-data/query/parameters` 与 `POST /api/v2/mass-data/query/instructions`；6.4.1/6.4.2.3 新增 `ConditionRangeEvaluator` 与 `IConditionHistoryProvider` 并保留 `ruleTree` 兼容路径；文档内海量接口前缀统一为 `/api/v2/mass-data` |
