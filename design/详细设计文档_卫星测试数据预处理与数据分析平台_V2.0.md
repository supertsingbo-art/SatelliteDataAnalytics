...
| `TASK_WINDOW_INVALID` | 400 | 一阶段 | 任务创建时 `window_start` ≥ `window_end`（两者均给出时由编排层校验） |
...
- `cached_parameter_count`：最近一次成功从海量 `POST /api/mass-data/basic/parameters` 拉取并写入本行对应 `param_cache` 的条数（与前端「参数同步总量」一致）。
- `cached_command_count`：最近一次成功从海量 `POST /api/mass-data/basic/commands` 拉取并写入本行对应 `command_cache` 的条数（与前端「指令同步总量」一致）。

**落库约定（6.1）**：海量接口服务、卫星测试流程规划服务经 `AssetSyncService` 同步的数据**默认写入 PostgreSQL**（`ConnectionStrings:Postgres`，表 `satellite_cache` / `param_cache` / `command_cache` / `test_batch_cache`）。应用配置 `AssetCache:UsePostgreSql` 默认为 `true`；仅本地无库联调时可设为 `false` 使用进程内 `InMemoryAssetCacheRepository`。表结构与本 DDL 对齐；也可预先执行仓库 `db/asset_cache_schema.sql`，否则由 `PgAssetCacheRepository` 在首次访问时 `CREATE TABLE IF NOT EXISTS` 自动建表。

#### 4.1.3 参数缓存表 `param_cache`
> 与海量 `POST /api/mass-data/basic/parameters` 响应对齐：常用字段结构化落列，其余字段保留在 `raw_json`。主键为海量 `paraId`（整型），不再使用历史 varchar `param_id` / `param_name` 列。
...
领域模型 `ParamCache` 提供计算属性 `ParamId => ParaId.ToString()`，供筛选模板 `config_json`、任务快照等仍以字符串形式引用参数主键的场景使用（与库列 `para_id` 数值一致）。

#### 4.1.3a 指令缓存表 `command_cache`
> 与参数缓存对称，承载海量基础库「指令」元数据（`POST /api/mass-data/basic/commands`），供后续判读、模板与任务编排引用；一阶段以同步与资产中心展示为主。
...
领域模型 `CommandCache` 提供计算属性 `CommandId => CmdId.ToString()`、`CommandName => CmdCode ?? CmdDesc ?? CommandId`（无独立 `command_id` / `command_name` 库列）。

入站接口：`POST /api/mass-data/basic/commands`，请求体同参数接口（`taskNo` / `satNo` / `dbStage` 三元组）。同步时按星先 `DELETE` 该星既有行再 `INSERT` 本次响应全量（「删后插」快照），成功后回写 `satellite_cache.cached_command_count`。
...
| 阶段 | 路由 | 页面 | 权限 |
|---|---|---|---|
| 一阶段 | `/assets/sources` | 数据源配置 | Admin |
| 一阶段 | `/assets/satellites` | 数据同步（卫星 / 参数 / 指令 / 测试阶段 / Mongo 缓存） | Admin / Analyst / Readonly |
| 一阶段 | `/assets/groups` | 卫星分组管理 | Admin / Analyst |
...
| 一阶段 | `/tasks/pipeline` | 新建 PIPELINE（可选资产 test-phases 快捷时间窗） | Admin / Analyst |
| 一阶段 | `/tasks/preprocess` | 仅预处理入仓 PREPROCESS（同上） | Admin / Analyst |
...
### 5.2 一阶段页面范围
一阶段页面重点保障核心链路：
- 数据源配置中心：维护海量数据接口、卫星测试流程规划服务及内部存储配置；
- **数据同步**（`/assets/satellites`）：卫星列表、全量/单星资产同步；明细 Drawer 含「参数」「指令」「测试阶段」「Mongo」选项卡，参数/指令表头为中文且支持关键字分页查询（数据来自 `param_cache` / `command_cache`，API 不返回 `raw_json`）；
- **卫星分组管理**（`/assets/groups`）：左侧分组树默认全部展开（`expandedKeys` 随树数据刷新重算，避免仅 `defaultExpandAll` 在异步加载后折叠）；右侧成员与模板归属操作；
- 筛选模板维护：规则条件树、持续时长、边界缓冲、离群配置；
- 算法模板维护：React Flow 画布、内置算法节点、DAG 合法性校验；
- 任务中心：创建预处理/算法/流水线任务、查看状态、进度、错误；
- 卫星分组管理：维护多级分组树（含系统「默认根分组」、物化路径 `group_path`）、卫星归属、模板归属预览；
...
数据源配置中心是第一阶段的根基模块，负责将外部权威系统（海量数据接口服务、卫星测试流程规划服务）下发的卫星、参数、测试阶段、Mongo 连接配置同步到本地缓存，为预处理、算法、任务编排提供稳定的元数据入口。本节按「子模块 → 同步流程 → 接口字段映射 → 同步策略」四个层次细化。

#### 6.1.1 子模块拆分
| 子模块 | 类/接口 | 对接接口 / 数据 | 职责 |
|---|---|---|---|
| 配置管理 | `DataSourceConfigService` | `data_source_config` | 维护外部服务（海量数据接口、卫星测试流程规划）与内部存储（ClickHouse、MinIO、PG_META）配置；显式禁止录入 `MONGO` 类型 |
| 海量接口适配 | `MassDataApiClient` + `MassDataParameterMapper` / `MassDataCommandMapper` | `GET /api/mass-data/satellites`、`POST /api/mass-data/basic/parameters`、`POST /api/mass-data/basic/commands`、`POST /api/mass-data/satellite/config` | 解析响应 `datas[]`，映射为 `ParamCache` / `CommandCache`；`cmdId`/`paraId` 缺失的记录跳过 |
| 资产查询 | `AssetQueryService` | `GET .../satellites`、`.../params`、`.../commands`、`.../test-phases`、`.../mongo-info` | 只读分页查询本地缓存；列表 DTO 为 `ParamCacheView` / `CommandCacheView`（不含 `raw_json`） |
| 测试流程规划适配 | `SatelliteAssetApiClient` | `POST /api/testplan/teststages` | 按 `(taskNo, satNo, dbStage)` 获取每颗星测试阶段清单（`teststagename`、`fromdt`、`todt`） |
| 资产同步编排 | `AssetSyncService` | 上述两个 Client + `IAssetCacheRepository`（**默认** `PgAssetCacheRepository` 写 PostgreSQL；`AssetCache:UsePostgreSql=false` 时为内存实现） | 按「全量卫星 → 每星参数 → 每星指令 → 每星测试阶段 → 每星 Mongo 配置」编排同步；各步 upsert/replace 落 PG 四表；单星子步骤失败隔离；回写 `satellite_cache.cached_parameter_count` / `cached_command_count` |
...
> 与早期文档字面的「三步骤」相比，实现上已将「指令」独立为 Step 2b，并在 `satellite_cache` 上持久化两类元数据的同步条数，便于资产中心列表展示与对账。**生产与默认开发配置**下，序列图中 `PG` 节点即 `ConnectionStrings:Postgres` 所指向的库（可与 `task_run`、`hq_param_metadata` 等同库不同 schema，由部署决定）；`AssetCache:UsePostgreSql=false` 时仅用于无 PostgreSQL 的轻量联调。
...
sequenceDiagram
    ...
    Sync->>Mass: GET /api/mass-data/satellites
    ...
    loop 每颗卫星 (taskNo, satNo, dbStage)
        ...
        Sync->>Mass: POST /api/mass-data/basic/parameters {taskNo, satNo, dbStage}
        Mass-->>Sync: paramList
        Sync->>PG: upsert param_cache（按 para_id ON CONFLICT 合并）

        Note over Sync,Mass: Step 2b 拉取指令元数据
        Sync->>Mass: POST /api/mass-data/basic/commands {taskNo, satNo, dbStage}
        Mass-->>Sync: cmdList
        Sync->>PG: replace command_cache 该星快照

        Note over Sync,Sat: Step 3 拉取测试阶段
        Sync->>Sat: POST /api/testplan/teststages {taskNo, satNo, dbStage}
        Sat-->>Sync: phaseList
        Sync->>PG: upsert test_batch_cache（语义=测试阶段）

        Note over Sync,Mass: Step 4 拉取 Mongo 连接配置
        Sync->>Mass: POST /api/mass-data/satellite/config {taskNo, satNo, dbStage}
...
入站接口：`GET /api/mass-data/satellites`，无入参。
...
入站接口：`POST /api/mass-data/basic/parameters`（响应数组键优先 `datas`，兼容 `items` / `parameters` / `paras`）。请求体使用 `SatelliteLookupRequest`：

```json
{ "taskNo": "T001", "satNo": "S001", "dbStage": "DEV" }
```

实现类：`MassDataApiClient.GetParametersAsync` → `MassDataParameterMapper.Map`。

| 海量接口字段 | 落库字段 | 说明 |
|---|---|---|
| — | `tasook_no` / `satellite_no` | 同步上下文三元组中的型号/卫星代号 |
| `paraId` | `para_id` | **必填**；与 `(tasook_no, satellite_no)` 构成 PK；缺失则跳过该条 |
| `paraCode` | `para_code` | 参数代号 |
| `paraDesc` | `para_desc` | 参数描述 |
| `paraTypeDesc` | `para_type_desc` | 参数类型描述 |
| `minValue` / `maxValue` | `min_value` / `max_value` | 数值上下限 |
| `updateTime` | `update_time` | 更新周期(ms) |
| `procDesc` | `proc_desc` | 处理方法描述 |
| `prmSysId` | `prm_sys_id` | 所属系统 ID |
| 响应根或卫星侧版本 | `source_version` | 与 `dbStage` 联动 |
| 同步时刻 | `last_synced_at` | UTC |
| 整条记录 `item` | `raw_json` | 含 `paraType`、`validFlag`、`paraMask` 等未单独建列字段 |

**未单独建列**（仅 `raw_json`）：如 `paraType`、`paraTypeChar`、`valueDesc`、`validFlag`、`watchFlag`、`parameterType`、`editGroup`、`procId`、`paraMask` 等，详见 `doc/海量数据服务API说明文档.md`。

upsert 规则：按 `(tasook_no, satellite_no, para_id)` `ON CONFLICT DO UPDATE` 合并结构化列与 `raw_json`；**不按星全量删除**，历史存在但本次响应未出现的 `para_id` 行保留（未来可增 `deleted_at` 软删）。

##### 6.1.3.2a 指令元数据 → `command_cache`
入站接口：`POST /api/mass-data/basic/commands`（响应数组键优先 `datas`，兼容 `items` / `commands` / `cmds`）。请求体同 6.1.3.2。

实现类：`MassDataApiClient.GetCommandsAsync` → `MassDataCommandMapper.Map`。

| 海量接口字段 | 落库字段 | 说明 |
|---|---|---|
| — | `tasook_no` / `satellite_no` | 同步上下文 |
| `cmdId` | `cmd_id` | **必填**；PK 组成部分；缺失则跳过 |
| `cmdCode` | `cmd_code` | 指令代号 |
| `cmdDesc` | `cmd_desc` | 指令描述 |
| `cmdType` | `cmd_type` | 指令类型 |
| `cmdLen` | `cmd_len` | 指令长度 |
| `exeTime` | `exe_time` | 执行时间(ms) |
| `validFlag` | `valid_flag` | 有效标志 |
| `cmdSysId` | `cmd_sys_id` | 所属系统 ID |
| 版本字段 | `source_version` |  |
| 同步时刻 | `last_synced_at` |  |
| 整条记录 | `raw_json` | 含 `cmdData`、`cmdLevel`、`isStarMiddleCmd`、`singnl`、`allowCheckData` 等 |

**未单独建列**（仅 `raw_json`）：`cmdData`、`cmdLevel`、`isStarMiddleCmd`、`singnl`、`allowCheckData` 等。

同步策略：按星 `DELETE` 后批量 `INSERT`（快照替换）；成功后更新 `satellite_cache.cached_command_count`。

##### 6.1.3.2b 资产缓存只读查询（API 出参）
| 方法 | 路径 | 响应 DTO | 说明 |
|---|---|---|---|
| GET | `/api/v1/asset/satellites` | `PagedResult<SatelliteListItem>` | 含 `cachedParameterCount` / `cachedCommandCount`、测试阶段标签 |
| GET | `/api/v1/asset/satellites/{tasookNo}/{satelliteNo}` | `SatelliteCache` | 单星摘要 |
| GET | `/api/v1/asset/satellites/{tasookNo}/{satelliteNo}/params` | `PagedResult<ParamCacheView>` | 关键字匹配 `paraId` / `paraCode` / `paraDesc` / `paraTypeDesc` / `procDesc` |
| GET | `/api/v1/asset/satellites/{tasookNo}/{satelliteNo}/commands` | `PagedResult<CommandCacheView>` | 关键字匹配 `cmdId` / `cmdCode` / `cmdDesc` |
| GET | `/api/v1/asset/satellites/{tasookNo}/{satelliteNo}/test-phases` | `TestBatchCache[]` | 测试阶段列表 |
| GET | `/api/v1/asset/satellites/{tasookNo}/{satelliteNo}/mongo-info` | `MongoConnectionSummary` | URI 脱敏 |

`ParamCacheView` / `CommandCacheView` 与表结构化列一致，**不包含** `raw_json`，供前端数据同步明细表展示。

##### 6.1.3.3 测试阶段 → `test_batch_cache`
入站接口：卫星测试流程规划服务 `POST /api/testplan/teststages`（参见 9.2）；请求体须含海量三元组 `taskNo` / `satNo` / `dbStage`。

| 规划服务字段 | 落库字段 | 说明 |
|---|---|---|
| `teststagename` | `scenario` | 测试阶段名称；若无独立编号则同时作为 `test_batch_id` |
| `fromdt` | `start_ts` | 阶段开始 UTC |
| `todt` | `end_ts` | 阶段结束 UTC |
...
入站接口：`POST /api/mass-data/satellite/config`，请求体为 `SatelliteLookupRequest`。

| 海量接口字段 | 落库字段 | 说明 |
|---|---|---|
| `mongoQueryConn` | `mongo_uri` | 写入时去除嵌入的密码；密码以引用形式入 `mongo_auth_ref` |
| `dbName`（解析自 URI 或额外字段） | `mongo_db_name` |  |
...
##### 幂等与增量
- `satellite_cache` / `test_batch_cache`：upsert，不删除历史记录；
- `param_cache`：按 `(tasook_no, satellite_no, para_id)` 逐条 upsert，不按星清空；未出现在本次海量响应中的历史 `para_id` **保留**（与「删后插」的指令策略不同）；
- `command_cache`：按星删后插，与海量当次 `datas` 快照一致；
...
#### 6.1.5 持久化实现与 schema 迁移
| 组件 | 说明 |
|---|---|
| `PgAssetCacheRepository` | 默认实现；`SchemaSql` 在首次访问时 `CREATE TABLE IF NOT EXISTS`，并对 `param_cache` / `command_cache` 自 V2.0.6 前 varchar 主键做 **ALTER 迁移**（`param_id`→`para_id`、`command_id`→`cmd_id`） |
| `InMemoryAssetCacheRepository` | `AssetCache:UsePostgreSql=false` 时用于无 PostgreSQL 联调；字典键分别为 `(tasook, satellite, paraId)` / `(tasook, satellite, cmdId)` |
| `db/asset_cache_schema.sql` | 与 `PgAssetCacheRepository` DDL 对齐的参考脚本，可手工预建表 |

PostgreSQL 仓储另含 `PgSatelliteGroupRepository` 等，与资产四表同库或分 schema 由部署决定（见 `ConnectionStrings:Postgres`）。
...
分组与卫星的对应关系由管理员在「资产 → 卫星分组」页面（`/assets/groups`）维护，不影响 `satellite_cache` 自身的同步流程。表结构详见 4.1.5a。

**后端（`SatelliteGroupService`）**：
- 系统初始化创建 **默认根分组**（`parent_group_id IS NULL`，名称「默认根分组」），不可删除、不可指定父级；
- `GET /asset/groups` 经 `BuildTree` 组装树：根为全部 `parent_group_id` 为空节点；若存在父节点缺失的孤儿节点，挂到默认根下，避免前端无法展示；
- 新建/移动分组时 `ResolveParentAsync`：`parentGroupId` 为空则挂到默认根；禁止成环。

**前端（`SatelliteGroupsPage`）**：
- 左侧 `<Tree>` 使用受控 `expandedKeys`，在树数据加载/刷新后收集全部节点 key 并展开（避免 `defaultExpandAll` 仅在首屏生效、异步数据到达后折叠）；
- 新建/编辑弹窗父分组 `<TreeSelect treeDefaultExpandAll />`；根分组编辑时禁用父级选择。
...
flowchart TB
    ...
    B["1. 有效时间段提取规则<br/>逻辑组合（AND/OR/NOT）+ 持续时长约束"]
...
| 参考卫星（适用数据范围） | **下拉单选**（选项 = 当前分组及其子分组下的成员星） | 必选；决定 `ruleTree` 与 `targetParams` 中 `param_id` 所绑定的 `param_cache` 来源星（`tasook_no`+`satellite_no`）。切换分组时若原参考星不在新分组成员内则清空重选 |
...
- 逻辑组合：参数条件可以多行，行间通过下拉切换 `AND` / `OR`；支持嵌套组形成 `ruleTree`；
- 单条参数条件：`参数（来自所选参考星的 param_cache）`+`比较符（>, >=, <, <=, ==, !=, between）`+`阈值`；
- 持续时长约束：以上条件需连续满足 N 秒才算有效段（`durationSeconds`）；
- 时间窗模式：默认按测试阶段（`mode=TEST_BATCH`），可配前后边界缓冲秒数（兼容原型 `bufferBeforeSeconds` / `bufferAfterSeconds`）；亦可配置为按任务显式时间窗（`mode=CUSTOM`，见下节）。

##### 6.2.3.3a 有效时间窗与任务入参、预处理入仓（与实现对齐）

预处理阶段在确定**有效时间窗** `EffectiveWindow [start, end]` 后，对 `targetParams` 中每个参数在 `[start, end]` 内从 Mongo 读取原始时序，执行离群打标，将点行批量写入 ClickHouse 表 `hq_param_point`，并在 PostgreSQL 表 `hq_param_metadata` 写入该参数本次任务的元数据（含时间窗起止、筛选模板版本、离群方法等）。**有效时间窗的来源**由筛选模板 `config_json.timeWindow.mode` 决定，与任务表 `task_run.window_start` / `task_run.window_end` 的关系如下：

| `timeWindow.mode` | 有效时间窗计算 | 对 `task_run.window_start` / `window_end` 的使用 |
|---|---|---|
| `TEST_BATCH` | 在资产缓存 `test_batch_cache` 中按 `task_run.test_batch_id` 匹配阶段，取 `start_ts` / `end_ts`，再按模板配置加减 `bufferBeforeSeconds` / `bufferAfterSeconds` | **不参与**有效窗计算；任务上仍可落库该字段用于审计或前端展示 |
| `CUSTOM` | 直接使用任务创建请求中的 `window_start` / `window_end`（须成对给出，且开始早于结束；编排层校验） | **必须**参与；与 6.1 资产接口 `GET /api/v1/assets/satellites/{tasook_no}/{satellite_no}/test-phases` 返回的阶段起止一致时，可由前端作为快捷填入，也支持用户手改 |

> **与界面交互的对应关系**：任务编排页「加载测试阶段」即调用上述 `test-phases` 接口；选择阶段后自动写入 `test_batch_id` 与起止时间；提交 `POST /api/v1/tasks/pipeline` 或 `POST /api/v1/tasks/preprocess` 时携带 `window_start` / `window_end`（ISO8601）。若两者皆为空且模板为 `CUSTOM`，创建将失败（评估器要求成对时间窗）；若模板为 `TEST_BATCH`，则须提供有效 `test_batch_id` 且资产侧已缓存该阶段。

> **规则树与持续时长**：模板中的 `ruleTree`、`durationSeconds` 表达「按条件切有效段」的产品语义；当前一阶段运行实现以**时间窗 + 目标参数列表**驱动 Mongo 区间读取与入仓。规则树逐点裁剪与持续时长裁剪可作为后续迭代增强点，不改变本节对 ClickHouse / PostgreSQL 落库职责的描述。

**目标参数与质量规则块**：表格形式，每行一个参数：

| 列 | 说明 |
|---|---|
| 提取目标参数 | 从**参考星**的 `param_cache` 选择，显示 `para_id` + `para_code`/`para_desc`（界面可仍标为参数 ID/名称） |
...
约束：
- 至少 1 个目标参数才允许保存；
- 同一 `param_id`（配置 JSON 内字符串，等于 `para_id`）在表格中不可重复；
...
```json
{
  ...
  "ruleTree": {
    "op": "AND",
    "children": [
      { "paramId": "A100_CURR_23", "operator": ">", "value": 5.0 },
      { "paramId": "A100_TEMP_PACK1", "operator": "<", "value": 80 }
    ]
  },
  "durationSeconds": 10,
  "targetParams": [
...
`timeWindow.mode=CUSTOM` 时示例（仅示意 `timeWindow` 块，其余字段同前）：
...
当**成员星 B** 选用已在**参考星 A** 上编制好的组级筛选模板时，`ruleTree` 与 `targetParams` 中落库的 `param_id` 仍为 A 侧主键。若 B 与 A 为「同组内、参数含义一致但编码不同」的典型情况，任务编排或运行时在落库任务快照**之前**应调用解析接口，将配置中的 `param_id` 重写为 B 在 `param_cache` 中的主键，避免同名/同描述不同码导致的错配。

| 步骤 | 说明 |
|---|---|
| 1 | 读取模板 `config_json` 中的 `scope.referenceTasookNo` / `referenceSatelliteNo` 作为参考星 A |
| 2 | 若目标星即为 A，直接消费原 `config_json`，无需映射 |
| 3 | 否则拉取 A、B 两侧 `param_cache`；对配置中出现的每个 `param_id`（字符串，等于参考星 `para_id` 的十进制表示）：若 B 上已存在同 `para_id` 则沿用；否则按 **`para_code` / `para_desc`**（或计算属性 `ParamName`）去首尾空白、忽略大小写唯一匹配；再否则按 `raw_json` 中 `paramDesc` / `description` / `desc` 等做忽略大小写匹配 |
...
- `tasook_no` / `satellite_no` / `test_batch_id`：来自 `task_run`；
- `window_start` / `window_end`：来自 `task_run.window_start/end`，未指定时按 `test_batch_cache` 的阶段起止反查。
...
#### 6.3.1.1 任务创建、时间窗与 PREPROCESS-only

- **内部 API**：`POST /api/v1/tasks/pipeline` 创建 `job_type=PIPELINE` 任务（预处理成功后入队算法阶段）；`POST /api/v1/tasks/preprocess` 创建 `job_type=PREPROCESS` 任务（仅执行预处理与终态 Webhook，**不入队**算法阶段）。二者请求体均支持可选字段 `test_batch_id`、`window_start`、`window_end`（ISO8601）；编排层在两者均给出时校验 `window_start < window_end`，否则返回 `400` / 错误码 `TASK_WINDOW_INVALID`。
- **开放 API（OAuth2 客户端凭证）**：`POST /openapi/v1/jobs/pipeline`、`POST /openapi/v1/jobs/preprocess`，鉴权与数据范围校验与内部创建语义一致；非法时间窗返回 OAuth 错误体 `invalid_request`。
- **幂等**：默认幂等键哈希包含型号、卫星、测试阶段、时间窗、模板版本等字段；`PREPROCESS` 与 `PIPELINE` 使用不同前缀，避免同参误合并。
- **前端**：`/tasks/pipeline`、`/tasks/preprocess` 支持从 `GET /api/v1/assets/satellites/{tasook_no}/{satellite_no}/test-phases` 加载阶段列表，快捷写入 `test_batch_id` 与 `DatePicker.RangePicker` 时间窗；亦可全手填。时间窗留空且模板为 `TEST_BATCH` 时，仍由阶段缓存决定有效窗；模板为 `CUSTOM` 时须提交成对时间窗。
...
**PREPROCESS-only 分支说明**：创建 `PREPROCESS` 任务时仍入队同一 `PreprocessJob`；预处理成功后**不再**入队 `AlgorithmJob`，直接将 `task_run` 置为成功并投递 `WebhookJob`。`AlgorithmJob` 若因异常被误调度，运行时应识别 `job_type != PIPELINE` 并立即返回，避免写入 `algo_result`。
...
> **实现补充（PIPELINE 与 PREPROCESS）**：`PIPELINE` 任务在预处理结束、算法尚未完成时，进度可处于预处理上沿附近；算法完成后进入 Webhook 前接近 100%。`PREPROCESS` 无算法阶段时，预处理完成后进度处于「预处理上沿～Webhook 前」区间，Webhook 完成后记为 100%，与 6.3.4 表格区间兼容表述，细粒度以发布版本代码 `TaskProgressBands` 为准。
...
当前一阶段实现入口为 `PreprocessPipeline`（Hangfire `PreprocessJob` 调用）。流程要点：**读取 `task_run` 与筛选模板版本 → `FilterRuleEvaluator` 依据模板 `timeWindow` 与任务字段计算有效时间窗及 `targetParams` → 拉取 Mongo 原始序列（时间落在窗内）→ 离群打标 → 批量写入 ClickHouse `hq_param_point` → 写入 PostgreSQL `hq_param_metadata` → 更新 `task_run` 进度；`PIPELINE` 再入队算法，`PREPROCESS` 则入队 Webhook 并终态成功**。

```mermaid
flowchart TD
    A[开始 PreprocessJob] --> B[读取 task_run 与筛选模板版本]
    B --> C[FilterRuleEvaluator 计算有效时间窗与 targetParams]
    C --> D[MongoConnectionPool 获取连接]
    D --> E[窗内读取各 param 原始序列]
    E --> F{有有效数据?}
    F -- 否 --> H[任务失败 PRE_001]
    F -- 是 --> I[离群值检测]
    I --> J[构造 hq_param_point]
    J --> K[批量写入 ClickHouse]
    K --> L[写 hq_param_metadata 至 PostgreSQL]
    L --> M[更新任务进度]
    M --> N{job_type}
    N -- PIPELINE --> O[入队 AlgorithmJob]
    N -- PREPROCESS --> P[入队 WebhookJob 成功终态]
```

> 图中「规则树 / 持续时长」的逐点裁剪若与模板 JSON 完整语义对齐，将在后续版本增强；当前以时间窗 + 目标参数驱动入仓，与 6.2.3.3a 说明一致。
...
> **与算法引擎判读节点的边界**：本节方法在**预处理阶段**对 `hq_param_point.is_outlier` 仅做**打标**，不剔除、不输出独立结果，相关元数据写入 `hq_param_metadata`。算法引擎中的 `threshold_judge` / `three_sigma_judge`（见 6.5.2.2）则在**算法阶段**对已打标的高品质数据产出**判读结果**，写入 `algo_result`。两者底层数学实现可复用，但触发位置、写入表与语义不同，可同时存在不冲突。
...
| 资产查询 | GET | `/asset/satellites` | 查询卫星缓存（支持 keyword 分页） |
| 资产查询 | GET | `/asset/satellites/{tasookNo}/{satelliteNo}` | 单颗卫星缓存摘要 |
| 资产查询 | GET | `/asset/satellites/{tasookNo}/{satelliteNo}/params` | 查询参数缓存（`ParamCacheView`，分页/keyword） |
| 资产查询 | GET | `/asset/satellites/{tasookNo}/{satelliteNo}/commands` | 查询指令缓存（`CommandCacheView`，分页/keyword） |
| 资产查询 | GET | `/asset/satellites/{tasookNo}/{satelliteNo}/test-phases` | 查询测试阶段（替代旧 test-batches） |
| 资产查询 | GET | `/asset/satellites/{tasookNo}/{satelliteNo}/mongo-info` | Mongo 连接摘要（URI 脱敏） |
...
响应为参数定义清单（`datas[]`，字段以 `paraId` 为主键），映射规则见 **6.1.3.2**，按 `(tasook_no, satellite_no, para_id)` upsert 到 `param_cache`。

> 说明：`POST /api/mass-data/query/parameters` 用于历史遥测数据值查询，由预处理引擎（6.4 节）在任务运行时使用，不参与资产同步。

#### 9.1.2a 查询指令元数据
```http
POST {mass_data_api}/api/mass-data/basic/commands
Content-Type: application/json
```

请求体同 9.1.2。响应为指令定义清单（`datas[]`，`cmdId` 为 int 主键），映射规则见 **6.1.3.2a**，按星删后插写入 `command_cache`。
...
- `dbName` → `satellite_cache.mongo_db_name`
- 密钥引用 → `satellite_cache.mongo_auth_ref`（不落明文密码）

### 9.2 卫星测试流程规划服务协议

卫星测试流程规划服务是一阶段必须对接的「测试阶段」权威来源（落库表沿用 `test_batch_cache`）。Web API 基础路由为 `/api/testplan`（详见 `doc/卫星测试流程规划服务API说明文档.docx`）。

**获取卫星测试阶段信息列表**

```http
POST {testplan_service}/api/testplan/teststages
Content-Type: application/json
...
```json
{
  "datas": [
    {
      "teststagename": "A1阶段",
      "fromdt": "2026-01-19T00:00:10Z",
      "todt": "2026-03-19T00:00:10Z"
    }
  ]
}
```
...
字段映射：
- `teststagename` → `test_batch_cache.scenario`；无独立编号时 → `test_batch_id`
- `fromdt` / `todt` → `start_ts` / `end_ts`
- 列表型响应亦可能为信封结构 `{ "taskNo", "satNo", "datas": [...] }`，客户端需兼容
...
| V2.0.7 | 2026-05-12 | 任务与时间窗：新增 6.2.3.3a（`TEST_BATCH`/`CUSTOM` 与 `task_run` 时间字段关系、预处理入 ClickHouse/PostgreSQL 说明、规则树实现边界）；6.2.3.4 增补 `CUSTOM` 的 `timeWindow` 示例 JSON；6.3.1.1（PIPELINE/PREPROCESS 创建 API、OAuth、幂等、前端 test-phases 快捷填入）；6.3.3 补 PREPROCESS-only 分支；6.3.4 补进度实现说明；6.4.2 流程图与文字与实现对齐；5.1 路由表补 `/tasks/pipeline`、`/tasks/preprocess` |
| V2.0.8 | 2026-05-12 | 6.1 资产同步：明确海量/资产服务同步数据**默认落 PostgreSQL**（四张 cache 表）；4.1.2 落库约定；`AssetCache:UsePostgreSql` 默认 true；实现 `AssetCacheOptions` + 启动校验 `ConnectionStrings:Postgres` |
| V2.0.9 | 2026-05-12 | 术语：6.1「资产配置中心」改「数据源配置中心」；「卫星资产」改「卫星测试流程规划」；9.2 对齐 `POST /api/testplan/teststages`；后端 `SatelliteAssetApiClient` 按规划服务 API 文档改造 |
| V2.1.0 | 2026-05-15 | **资产缓存结构化**：`param_cache` 主键改为 `(tasook_no, satellite_no, para_id)`，落列 `para_code`/`para_desc`/`para_type_desc`/`min_value`/`max_value`/`update_time`/`proc_desc`/`prm_sys_id`；`command_cache` 主键改为 `(tasook_no, satellite_no, cmd_id)`，落列 `cmd_code`/`cmd_desc`/`cmd_type`/`cmd_len`/`exe_time`/`valid_flag`/`cmd_sys_id`；其余海量字段入 `raw_json`。实现 `MassDataParameterMapper`、`MassDataCommandMapper`；`PgAssetCacheRepository` 迁移 DDL + `GetCommandsAsync`；新增 `ParamCacheView`/`CommandCacheView` 与 `GET .../params`、`GET .../commands`。前端数据同步页增加「指令」Tab（中文列）；卫星分组树默认展开（受控 `expandedKeys`）。5.1 补 `/assets/groups`；5.2/6.1.3/6.1.4/6.2.3.2/8.2/9.1 同步 |
