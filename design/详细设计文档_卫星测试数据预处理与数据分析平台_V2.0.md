...
入站接口：`POST /api/mass-data/basic/commands`，请求体同参数接口（`taskNo` / `satNo` / `dbStage` 三元组）。同步时按星先清空该星在 `command_cache` 中的旧行再写入本次响应全量（与参数 upsert 策略可分别演进，当前实现以「删后插」保证与接口快照一致）。
...
| 一阶段 | `/assets/satellites` | 卫星测试流程规划 | Admin / Analyst / Readonly |
...
- 卫星测试流程规划：展示卫星、参数、测试阶段与 Mongo 摘要；
...
| 海量接口适配 | `MassDataApiClient` | `GET /api/mass-data/satellites`、`POST /api/mass-data/basic/parameters`、`POST /api/mass-data/basic/commands`、`POST /api/mass-data/satellite/config` | 封装 Web API 调用、超时与重试、错误码映射 |
...
入站接口：`POST /api/mass-data/basic/parameters`，请求体使用 `SatelliteLookupRequest`：
...
入站接口：`POST /api/mass-data/basic/commands`，请求体同 6.1.3.2（`SatelliteLookupRequest` 三元组）。
