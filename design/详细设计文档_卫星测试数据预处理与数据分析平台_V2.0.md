...
| `IConditionHistoryProvider` / `MongoConditionHistoryProvider` | 运行期从**任务星** Mongo（`task_run` 对应 `satellite_cache` 连接）读取条件历史；参数/指令 ID 元数据仍来自**参考星** `param_cache` / `command_cache` |
...
**条件历史数据来源**（运行期，与 ruleTree 时代一致：任务星 Mongo + 参考星元数据）：
- Mongo 连接：任务执行星 `task_run.tasook_no` / `satellite_no` 对应 `satellite_cache.mongo_uri` / `mongo_db_name`；
- 参数条件：`MongoPkgSeriesReader` → 集合 `pkg{prm_sys_id}`（`paraId`/`prmSysId` 来自参考星 `param_cache`）；
- 指令条件：`MongoInstructionSeriesReader` → 集合 `IndicatorCollection`（`cmdId`/`cmdSysId` 来自参考星 `command_cache`）；
...
| V2.0.11 | 2026-06-17 | **条件历史回退 Mongo 直读**：6.4.1/6.4.2.3 将运行期条件参数/指令历史从海量 `query/parameters`、`query/instructions` 改回 Mongo（`pkg{prm_sys_id}` + `IndicatorCollection`）；新增 `MongoConditionHistoryProvider`、`MongoInstructionSeriesReader`；Mongo 连接取自**任务星**、参数/指令 ID 元数据取自**参考星**（与 ruleTree 时代一致）；9.1 明确 query 接口不参与本平台预处理；资产同步仍使用 v2 `basic/*` + `satellite/config` |
