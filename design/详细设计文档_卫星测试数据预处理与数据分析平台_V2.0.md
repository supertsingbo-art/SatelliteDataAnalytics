...
create table task_run (
    ...
    -- 预处理 Mongo 拉取与 ruleTree 评估的数据时间范围（创建任务时确定，执行期只读）
...
- 持续时长约束：逻辑组合后的时间段须连续满足 ≥ `durationSeconds` 秒才保留（见 6.4.2.3）；
...
- **无参数条件**：`ruleTree` 为 `{ "op": "AND", "children": [] }` 或等价空树时，目标参数有效窗 = 任务数据时间窗（`task_run.window_start` ~ `window_end`），与 6.4.2.3 一致。

**`ruleTree` 运行时求值算法（实现：`RuleTreeSegmentEvaluator.ComputeValidRanges`）** 见 6.4.2.3：先按各条件参数在任务窗内各自筛段，再对时间段做 AND（交集）/ OR（并集）/ NOT（任务窗内补集）。
...
| `RuleTreeSegmentEvaluator` | 由 `ruleTree` + 条件参数时序计算 **目标参数有效时间段**（区间交/并/补，见 6.4.2.3） |
| `MongoPkgSeriesReader` | 按 `prm_sys_id` 读取 Mongo `pkg{sysId}` 集合 |
| `OutlierDetector` | 离群值检测 |
| `ClickHouseBatchWriter` | 批量写入 `hq_param_point` |
| `MetadataWriter` | 写 `hq_param_metadata`、`preprocess_outlier_segment` |
...
flowchart TD
    ...
    D --> E[加载参考星条件参数时序 / 目标星参数时序]
    E --> F{ruleTree 含参数条件?}
    F -- 否 --> F1[目标有效段 = 任务数据全窗]
    F -- 是 --> F2[RuleTreeSegmentEvaluator: 按 6.4.2.3 求有效段]
    F1 --> G
    F2 --> G{有效段非空?}
    G -- 否 --> H[失败 PRE_004]
...
| 字段 | 含义 | 预处理中的用法 |
|---|---|---|
| `window_start` / `window_end` | 任务**数据时间范围**（创建/调度时确定） | **唯一**用于 Mongo 拉取范围、`ruleTree` 评估范围、`FilterRuleEvaluator` 输入；立即任务 = 用户所选时段；每天定时 = 前一日同刻 ~ 当日计划执行同刻（写入 `task_run`） |
...
- 输出 `EffectiveWindow` 供条件参数与目标参数读取 Mongo 时的**外边界**；目标参数若在 `ruleTree` 下还有更窄的有效段，由 6.4.2.3 在此外边界内再裁剪。

#### 6.4.2.3 目标参数有效时间段（`RuleTreeSegmentEvaluator`）

在 **任务数据时间窗** `[window_start, window_end]` 内，对 `ruleTree` 求值，得到一或多个 `{Tmin, Tmax}`，供目标参数提取与边界缓冲（`targetParams.boundaryBufferBeforeSec/AfterSec`）使用。

**步骤（与实现对齐）**：

1. **按参数筛段（叶子）**  
   对每个叶子 `{ paramId, operator, value }`：仅使用该参数自身时序（窗起止 + 窗内采样时刻），逐时刻判断比较条件；连续满足区间合并为 `{Tmin, Tmax}`（可多段）。首采样点之前无数据，视为不满足。

2. **重复**  
   对 `ruleTree` 中出现的每个条件参数各得一组时间段。

3. **逻辑组合（区间运算）**  
   | `op` | 语义 | 实现 |
   |---|---|---|
   | `AND` | 各子树结果的**交集**（求交） | 两两 `IntersectRanges` |
   | `OR` | 各子树结果的**并集**（求并） | `UnionRanges`（合并重叠段） |
   | `NOT` | 任务窗内对唯一子树结果取**补集** | `ComplementRanges` |

4. **持续时长**  
   对步骤 3 合并后的每段，若 `durationSeconds > 0`，丢弃时长 `< durationSeconds` 的段；无满足段时任务失败 `PRE_004`（不回退为全任务窗）。

5. **无 `ruleTree` 参数条件**  
   有效段 = `[window_start, window_end]` 单段。

**条件参数时序来源**：`ruleTree` 中 `paramId` 所属参考星（`config_json.scope.referenceTasookNo/referenceSatelliteNo`）的 Mongo 序列；目标参数使用任务卫星本星序列。
...
| 层级 | 字段/表 | 含义 |
|---|---|---|
| 算法候选 | `hq_param_point.is_outlier=1` | 预处理自动检出，保留审计 |
| 人工状态 | `preprocess_outlier_point_review.review_status` | `PENDING` / `CONFIRMED` / `JITTER`（抖动，非真离群） |
| 最终离群 | `hq_param_point.is_confirmed_outlier=1` | 用户确认后 CH 插入更高 `version` |
| 时间段 | `preprocess_outlier_segment.segment_kind` | 复核前展示 `AUTO`；`POST complete` 后写入 `CONFIRMED` |

**流程**：预处理成功 → 种子写入 `preprocess_outlier_point_review`（`PENDING`）并更新 `task_run.outlier_*` 计数 → 用户在任务列表「数据明细 / 离群复核」逐条或批量确认 → `PATCH /api/v1/tasks/{runId}/outlier-reviews` → 全部处理完后 `POST .../outlier-reviews/complete` 重算确认段。

**接口**（`TasksController`）：
- `GET .../outlier-reviews/summary`
- `GET .../outlier-reviews?status=&paramId=&page=&pageSize=`
- `PATCH .../outlier-reviews` body `{ items: [{ paramId, ts, status: CONFIRMED|JITTER }] }`
- `POST .../outlier-reviews/complete`（要求 `pending_count=0`）
...
| V2.0.8 | 2026-05-19 | **离群点人工复核**：新增 `preprocess_outlier_point_review`、`task_run.outlier_*` 计数、`preprocess_outlier_segment.segment_kind`；CH `hq_param_point.is_confirmed_outlier`；6.4.5 复核流程与 API；前端任务列表「离群复核」Tab（进度、筛选、批量确认/抖动、完成复核）；`OutlierReviewService` + 完成复核后重算 CONFIRMED 段 |
