    ...
    ILogger<PreprocessPipeline> logger) : IPreprocessPipeline
{
    ...
    private async Task FailWithParamConflictsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var details = await conflictEnricher.EnrichAsync(run, acquireResult.Conflicts, cancellationToken)
            .ConfigureAwait(false);
        var message = PreprocessConflictEnricher.BuildReadableMessage(details);
        message = $"{message}。当前策略(active={options.OnActiveConflict}, committed={options.OnCommittedConflict})";
